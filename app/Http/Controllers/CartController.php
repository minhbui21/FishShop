...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if($already_cart) {
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return back()->with('error','Vượt quá số lượng cho phép!.');
            $already_cart->save();
            ...
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error','Vượt quá số lượng cho phép!.');
            $cart->save();
    ...
    public function singleAddToCart(Request $request){
        ...
        if($product->stock <$request->quant[1]){
            return back()->with('error','Đã vượt quá số lượng còn lại của sản phẩm. Vui lòng chọn lại số lượng!');
        }
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    if($cart->product->stock < $quant){
                        request()->session()->flash('error','Đã vượt quá số lượng còn lại của sản phẩm. Vui lòng chọn lại số lượng!');
                        return back();
                    }
